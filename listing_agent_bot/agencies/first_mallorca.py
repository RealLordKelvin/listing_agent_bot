def first_mallorca_scraper(page_object):
    proeprties_cards = get_properties(page_object)
    listings = get_informations_of_properties(proeprties_cards)
    return listings


def get_properties(initial_search_page):

    proeprties_cards = initial_search_page.find('div', attrs = {'class':'row three narrow wrap grid'})

    return proeprties_cards

def get_informations_of_properties(proeprties_cards):

    listings = []
    for _property in proeprties_cards.findAll('div', attrs = {'class':'card'}):
        ...
        try:
            ...
            listing['reference_number'] = description.find('div', attrs = {'class':'ref_id right'}).get_text()
            # ... der listing preis...
